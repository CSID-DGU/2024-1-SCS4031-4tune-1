...
async def send_cheating_result(user_id: str):
    # 부정행위 데이터를 만들어서 전송 준비
    if user_id not in cheating_counts or not any(count > 0 for count in cheating_counts[user_id].values()):
        logging.error(f"Cheating result not found for user {user_id}")
        return  # 부정행위 데이터가 없으면 종료

    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    cheating_result = {
        "userID": user_id,
        "cheatingResults": cheating_counts[user_id],
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(BACKEND_API_URL, json=cheating_result) as resp:
                if resp.status == 200:
                    logging.info(f"Successfully sent cheating result for {user_id}")
                else:
                    logging.error(f"Failed to send cheating result for {user_id}, status: {resp.status}")
                    response_text = await resp.text()  # 서버에서 반환하는 텍스트 오류 메시지 확인
                    logging.error(f"Response from backend: {response_text}")
        except Exception as e:
            logging.error(f"Failed to send cheating result for {user_id}: {e}")
